(function(){
    ...
    var ID = 'filecropper';
    var CROPPER_BOX = "cropper-box"

    var Filecropper = function(__args__)
    {
      var _self = this;
      setTimeout(function(){
        _self.init(__args__);
    ...
    Filecropper.prototype.constructor = Filecropper;

    //-------
    Filecropper.prototype.init = function(__args__){

      var defaults = {
          headers: __args__.headers ? __args__.headers : [], //[]{key:'Authorization', value:'my-value-key'}]
          url: __args__.url ? __args__.url : "",
          target: __args__.target ? __args__.target : ".filecropper",
          editImage: __args__.editImage ? __args__.editImage : false,
          crop:{
              width:  __args__.crop.width ?__args__.crop.width : 300,
              height:  __args__.crop.height ? __args__.crop.height : 250,
              type: __args__.crop.type ? __args__.crop.type : 'square'
          },
          text:{
            noSUpport: __args__.text.noSUpport ? __args__.text.noSUpport : "This application requires modern browser support, please update yours.",
            btCancelText: __args__.text.btCancelText ? __args__.text.btCancelText : "Cancel",
            btSaveText: __args__.text.btSaveText ? __args__.text.btSaveText :"Save"
          },
          onComplete: __args__.onComplete ? __args__.onComplete : function(){},
          onFileSelected: __args__.onFileSelected ? __args__.onFileSelected : function(){},
          onRemoved: __args__.onRemoved ? __args__.onRemoved : function(){},
          onStartUpload: __args__.onStartUpload ? __args__.onStartUpload : function(){}
      };

      if(!window.File || !window.FileList || !window.FileReader){
        alert(defaults.text.noSUpport);
        return false;
      };
    ...
    Filecropper.prototype.getFileName = function(){ return this.fileName; }

    Filecropper.prototype.getFileInput= function(){ return this.fileInput; }

    Filecropper.prototype.getFileBlob = function(){ return this.fileBlob; }

    Filecropper.prototype.initInputEvents = function(){
      var _self = this;
      var _target = _self.parameters.target;
      var el = document.querySelector(_target);

      if(!el){
        throw new Error('Filecropper target doesn\'t exists : '+_target);
        return false;
      }
      ...
      el.onchange = function (evt) {
          ...
          if (files && files.length) {
            var file = files[0];
            var _type = file.type.match('image') ? 'image' : 'file';
            var _file = new File([file], file.name);
            _self.fileName = file.name;
            _self.fileInput = file;

            _self.parameters.onFileSelected({ target: tgt, file: file});

            if (!file.type.match('image')){
              _self.fileBlob = _file;
              _self.uploadFile({});
            } else {
              var _editImage = _self.parameters.editImage;
              if(_editImage){
                _self.editImage({ });
              } else {
                _self.fileBlob = _file;
    ...
    Filecropper.prototype.uploadFile = function(__args__){
      __args__ = __args__ ? __args__ : {};

      var _self = this;
      var _url = _self.parameters.url;
      var _headers = _self.parameters.headers;
      var formData = new FormData(),
      xhr = new XMLHttpRequest();

      _self.parameters.onStartUpload(__args__);

      formData.append('file', _self.fileBlob);
      xhr.open('POST', _url);

      if(_headers.length){
        for(var i=0; i<_headers.length; i++){
          xhr.setRequestHeader(_headers[i].key, _headers[i].value);
        }
      }
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          __args__.status = xhr.status;

          if(xhr.status == 200){
            var data = xhr.responseText;
            __args__.file = _self.fileBlob;
            __args__.response = data;
            _self.parameters.onComplete(__args__);
            _self.removeCropper();
          }

          if(xhr.status == 403){
            __args__.response = "Forbidden";
            _self.parameters.onComplete(__args__);
          }

          if(xhr.status == 404){
            __args__.response = "Page not found";
            _self.parameters.onComplete(__args__);
          }
    ...
    Filecropper.prototype.editImage = function(__args__){
      var _self = this;
      var _file = _self.fileInput;
      var _picReader = new FileReader();
      _picReader.onload = function(_event){
        _self.imageLoaded({ event: _event, file: _file });
      }
      _picReader.readAsDataURL(_file);
    }

    Filecropper.prototype.imageLoaded = function(__args__){
      var _self = this;
      var _event = __args__.event;
      var _picFile = _event.target;

      _self.createUI({});

      var _i = document.getElementById('filecropper__img');
      _i.src = _picFile.result;

      //create cropie
      var c = new Croppie(_i,
        {
            viewport: {
                width: _self.parameters.crop.width,
                height: _self.parameters.crop.height,
                type: _self.parameters.crop.type
            },
            enableExif: true,
            showZoomer: false,
      ...
      _self.buttonsAction(__args__);
    }

    Filecropper.prototype.buttonsAction = function(__args__){
      var _self = this;
      var c = _self.croppie;
      var zoomin = document.getElementById('filecropper__bt-zoomin');
      ...
      var zoomout = document.getElementById('filecropper__bt-zoomout');
      ...
      var rotateleft = document.getElementById('filecropper__bt-rotateleft');
      rotateleft.onclick = function(){
        c.rotate( 90);
        return false;
      }

      var rotateright = document.getElementById('filecropper__bt-rotateright');
      rotateright.onclick = function(){
        c.rotate(-90);
        return false;
      }

      var btcancel = document.getElementById('filecropper__btcancel');
      btcancel.onclick = function(){
        _self.removeCropper();
        return false;
      }

      var btsave = document.getElementById('filecropper__btsave');
      var file = __args__.file.name;
    ...
    Filecropper.prototype.removeCropper = function(){
      var _self = this;
      var cropper = document.getElementById(CROPPER_BOX);

      if(!cropper)
        return false;

      removeClass({ item: cropper, class: 'on'});

      setTimeout(function(){
        cropper.parentNode.removeChild(cropper);
        _self.parameters.onRemoved();
      },300);
    }

    Filecropper.prototype.createUI = function(__args__){
      var _self = this;
        var _html =
            "<div id='"+ID+"'>"+
                "<div class='filecropper__preview'><img id='filecropper__img' class='filecropper__img'></div>" +
                "<div class='filecropper__tools'>" +
                "<div class='filecropper__controll'>" +
                "<div class='filecropper__btn'>" +
                "<a href='#' class='filecropper__btn__bt filecropper__bt-zoomout' id='filecropper__bt-zoomout'><svg width='15' height='15' fill='#ffffff' viewBox='0 0 1792 1792' ><path d='M1088 800v64q0 13-9.5 22.5t-22.5 9.5h-576q-13 0-22.5-9.5t-9.5-22.5v-64q0-13 9.5-22.5t22.5-9.5h576q13 0 22.5 9.5t9.5 22.5zm128 32q0-185-131.5-316.5t-316.5-131.5-316.5 131.5-131.5 316.5 131.5 316.5 316.5 131.5 316.5-131.5 131.5-316.5zm512 832q0 53-37.5 90.5t-90.5 37.5q-54 0-90-38l-343-342q-179 124-399 124-143 0-273.5-55.5t-225-150-150-225-55.5-273.5 55.5-273.5 150-225 225-150 273.5-55.5 273.5 55.5 225 150 150 225 55.5 273.5q0 220-124 399l343 343q37 37 37 90z'/></svg></a>" +
                "<a href='#' class='filecropper__btn__bt filecropper__bt-zoomin' id='filecropper__bt-zoomin'><svg width='15' height='15' fill='#ffffff' viewBox='0 0 1792 1792' ><path d='M1088 800v64q0 13-9.5 22.5t-22.5 9.5h-224v224q0 13-9.5 22.5t-22.5 9.5h-64q-13 0-22.5-9.5t-9.5-22.5v-224h-224q-13 0-22.5-9.5t-9.5-22.5v-64q0-13 9.5-22.5t22.5-9.5h224v-224q0-13 9.5-22.5t22.5-9.5h64q13 0 22.5 9.5t9.5 22.5v224h224q13 0 22.5 9.5t9.5 22.5zm128 32q0-185-131.5-316.5t-316.5-131.5-316.5 131.5-131.5 316.5 131.5 316.5 316.5 131.5 316.5-131.5 131.5-316.5zm512 832q0 53-37.5 90.5t-90.5 37.5q-54 0-90-38l-343-342q-179 124-399 124-143 0-273.5-55.5t-225-150-150-225-55.5-273.5 55.5-273.5 150-225 225-150 273.5-55.5 273.5 55.5 225 150 150 225 55.5 273.5q0 220-124 399l343 343q37 37 37 90z'/></svg></a>" +
                "</div>" +
                "<div class='filecropper__btn'>" +
                "<a href='#' class='filecropper__btn__bt filecropper__bt-rotateleft' id='filecropper__bt-rotateleft'><svg width='15' fill='#ffffff' height='15' viewBox='0 0 1792 1792' xmlns='http://www.w3.org/2000/svg'><path d='M1664 896q0 156-61 298t-164 245-245 164-298 61q-172 0-327-72.5t-264-204.5q-7-10-6.5-22.5t8.5-20.5l137-138q10-9 25-9 16 2 23 12 73 95 179 147t225 52q104 0 198.5-40.5t163.5-109.5 109.5-163.5 40.5-198.5-40.5-198.5-109.5-163.5-163.5-109.5-198.5-40.5q-98 0-188 35.5t-160 101.5l137 138q31 30 14 69-17 40-59 40h-448q-26 0-45-19t-19-45v-448q0-42 40-59 39-17 69 14l130 129q107-101 244.5-156.5t284.5-55.5q156 0 298 61t245 164 164 245 61 298z'/></svg></a>" +
                "<a href='#' class='filecropper__btn__bt filecropper__bt-rotateright' id='filecropper__bt-rotateright'><svg width='15' fill='#ffffff' height='15' viewBox='0 0 1792 1792' ><path d='M1664 256v448q0 26-19 45t-45 19h-448q-42 0-59-40-17-39 14-69l138-138q-148-137-349-137-104 0-198.5 40.5t-163.5 109.5-109.5 163.5-40.5 198.5 40.5 198.5 109.5 163.5 163.5 109.5 198.5 40.5q119 0 225-52t179-147q7-10 23-12 15 0 25 9l137 138q9 8 9.5 20.5t-7.5 22.5q-109 132-264 204.5t-327 72.5q-156 0-298-61t-245-164-164-245-61-298 61-298 164-245 245-164 298-61q147 0 284.5 55.5t244.5 156.5l130-129q29-31 70-14 39 17 39 59z'/></svg></a>" +
                "</div>" +
                "</div>" +
                "<div class='filecropper__buttons'>" +
                "<a href='#' class='filecropper__bt filecropper__btcancel' id='filecropper__btcancel'>"+_self.parameters.text.btCancelText+"</a>" +
                "<a href='#' class='filecropper__bt filecropper__bt--fill filecropper__btsave' id='filecropper__btsave'>"+_self.parameters.text.btSaveText+"</a>" +
                "</div>" +
                "</div>"+
            "</div>";

      var _wrapper = document.createElement('div');
      _wrapper.id = CROPPER_BOX;
      _wrapper.innerHTML = _html;
      document.body.appendChild(_wrapper);
      setTimeout(function(){
        addClass({ item: _wrapper, class: "on"});
      },10);
    }

    function addClass(__args__){
      var d = __args__.item,
          c = __args__.class;
      if (d.classList)
        d.classList.add(c);
    ...
    window.Filecropper = Filecropper;
